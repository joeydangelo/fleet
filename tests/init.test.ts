...
describe('role-specific skill sources', () => {
  ...
  for (const role of roles) {
    it(`${role} skill source exists and has valid frontmatter`, () => {
      ...
      expect(content).toContain('allowed-tools: Bash(paw:*)');
    });
  }
});
...
describe('paw init — role-specific skills with filtered directories', () => {
  let repoRoot: string;
  ...
  it('installs three role-specific skills with injected directories', () => {
    ...
    expect(builderContent).toMatch(/\| `paw guidelines \S+` \|/);

    // Old monolithic skill should not exist
    expect(existsSync(resolve(repoRoot, '.claude', 'skills', 'paw', 'SKILL.md'))).toBe(false);
  });
});
